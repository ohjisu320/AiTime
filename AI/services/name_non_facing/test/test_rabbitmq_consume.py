#!/usr/bin/env python
"""
RabbitMQ 결과 큐에서 메시지 확인

Usage:
    python test/test_rabbitmq_consume.py           # 하나만 가져오기
    python test/test_rabbitmq_consume.py --wait    # 메시지 올 때까지 대기
    python test/test_rabbitmq_consume.py --all     # 모든 메시지 가져오기
"""
...
DEFAULT_QUEUE = "analysis.name_non_facing.result"
...
def main():
    parser = argparse.ArgumentParser(description="RabbitMQ 결과 큐 메시지 확인")
    parser.add_argument(
        "--wait", "-w",
        action="store_true",
        help="메시지가 올 때까지 대기",
