#!/usr/bin/env python
"""
RabbitMQ 요청 큐에 목업 메시지 발행

Usage:
    python test/test_rabbitmq_publish.py
    python test/test_rabbitmq_publish.py --video sample_video/name_calling.mp4
    python test/test_rabbitmq_publish.py --child-name "홍길동" --age 18
"""
...
DEFAULT_QUEUE = "analysis.name_non_facing.request"


def publish_mock_message(
    video_path: str = "sample_video/name_calling.mp4",
    ...
    message = {
        ...
        "video_path": str(video_file.absolute()),  # 절대 경로로 변환
    }
...
def main():
    parser = argparse.ArgumentParser(description="RabbitMQ 목업 메시지 발행")
    parser.add_argument(
        "--video", "-v",
        default="sample_video/name_calling.mp4",
        help="비디오 파일 경로 (기본: sample_video/name_calling.mp4)",
    )
    parser.add_argument(
        "--child-name", "-n",
        default="테스트아이",
