...
const ExamGuideVideoPage: React.FC = () => {
  ...
  if (!videoSrc) {
    return <div className="p-20 text-white font-['Noto_Sans_KR']">잘못된 접근입니다.</div>;
  } 

  // ✅ 다음 단계로 이동하는 공통 함수 [cite: 2026-01-27]
    ...
    >
      ...
      <div className="w-full h-full absolute inset-0 bg-gray-900 pointer-events-none">
        <video
          ...
          muted 
          playsInline
          onEnded={moveToNextStep} // 👈 영상 재생이 끝나면 자동으로 실행 [cite: 2026-01-27]
          // loop 속성은 제거했습니다. [cite: 2026-01-27]
        >
          <source src={videoSrc} type="video/mp4" />
        </video>
      </div>

      {/* 2. 중앙 텍스트 오버레이 [cite: 2026-01-27] */}
      <div className="z-10 pointer-events-none">
        <FullScreenOverlayText text={`사전\n안내영상`} />
      </div>

      {/* 3. 일시정지 상태 피드백 [cite: 2026-01-27] */}
      {isPaused && (
      ...
      {/* 4. 상단 뒤로가기 버튼 [cite: 2026-01-27] */}
      <div onClick={(e) => e.stopPropagation()} className="z-30">
        <GlassBackButton onClick={() => navigate('/exam/mission')} />
      </div>

      {/* 5. 하단 Skip 버튼 [cite: 2026-01-27] */}
      <button
        onClick={(e) => {
          e.stopPropagation();
          moveToNextStep(); // 👈 수동으로 넘어가기 [cite: 2026-01-27]
        }}
