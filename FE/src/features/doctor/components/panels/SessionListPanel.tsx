...
export default function SessionListPanel({ examVideoList, onSelectVideo, currentVideoExamId }: Props) {
  // [ADD] Padding Logic to ensure at least 5 items
  const buildPaddedList = (source: ExamVideoListItem[]): ExamVideoListItem[] => {
    ...
    const dummies: ExamVideoListItem[] = Array.from({ length: prependCount }, (_, idx) => ({
      examId: `padded-${idx}`,
      examDate: `${idx + 1}차 (데이터 없음)`,
      examStatus: "-",
      videos: []
    }));

    return [...dummies, ...source];
  };

  const displayList = buildPaddedList(examVideoList);
    ...
    <div className="p-2 h-full bg-white border border-[#808080] font-['Gulim'] overflow-y-auto">
      ...
      <div className="flex flex-col gap-4 mt-2 px-1">
        {displayList.map((exam) => {
          const isDummy = exam.examId.startsWith('padded-');
          // Open if it's the current exam OR if it's the first REAL exam (when no current is selected)
          // Actually, original logic was `examVideoList[0].examId === exam.examId`.
          // If we have dummies, examVideoList[0] is the first REAL item.
          // So we should check if `exam` matches `examVideoList[0]`.
          const isFirstReal = examVideoList.length > 0 && examVideoList[0].examId === exam.examId;

          const isOpen = !isDummy && (exam.examId === currentVideoExamId || isFirstReal);

          return (
            <details
              key={exam.examId}
              className={`group ${isDummy ? 'cursor-default opacity-50' : 'cursor-pointer'}`}
              open={isOpen}
            >
              <summary className={`font-bold text-[14px] select-none text-black ${!isDummy && 'group-hover:text-blue-900'} list-none mb-1`}>
                <span className={`inline-block w-4 mr-1 text-center ${!isDummy && 'group-open:rotate-90'} transition-transform`}>
                  {isDummy ? '·' : '▶'}
                </span>
