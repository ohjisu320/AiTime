...
import { Hospital as HospitalIcon, Plus, ChevronRight } from 'lucide-react';
...
const HospitalTimeline = ({ hospitals, childName }: HospitalTimelineProps) => {
  // 데이터가 정말로 있는지 확인하는 조건 (배열 길이가 0보다 커야 함)
  const hasHospitals = hospitals && hospitals.length > 0;

  return (
    <Card className="w-full h-full rounded-3xl shadow-xl border-gray-100 flex flex-col overflow-hidden bg-white">
      ...
      <CardContent className="flex-1 px-8 pb-8 flex flex-col">
        {hasHospitals ? (
          ...
          /* 2. 병원 목록이 없을 때 (Empty State): 버튼 노출 */
          <div className="flex-1 flex flex-col items-center justify-center text-center p-6 bg-gray-50/50 rounded-2xl border-2 border-dashed border-gray-200 my-4">
            ...
            <Dialog>
              <DialogTrigger asChild>
                <Button variant="outline" className="rounded-xl border-indigo-100 text-[#6366F1] font-bold hover:bg-indigo-600 hover:text-white transition-all text-xs px-6">
                  초대 코드 등록하기
                </Button>
              </DialogTrigger>
              <DialogContent className="rounded-3xl sm:max-w-md">
                <DialogHeader>
                  <DialogTitle className="text-2xl font-bold text-[#6366F1]">초대 코드 등록</DialogTitle>
                  <DialogDescription>병원에서 받은 6자리 코드를 입력하세요.</DialogDescription>
                </DialogHeader>
                <div className="py-6">
                  <Input placeholder="ABC123" className="h-14 text-center text-xl font-mono rounded-2xl focus-visible:ring-[#6366F1]" />
                </div>
                <Button className="w-full h-14 bg-[#6366F1] hover:bg-[#4F46E5] rounded-2xl font-bold text-lg">등록 완료</Button>
              </DialogContent>
            </Dialog>
          </div>
        )}
        ...
        <Button variant="ghost" className="w-full h-14 mt-auto bg-gray-50 text-gray-500 rounded-2xl font-bold hover:bg-gray-100">
          전체 활동 보기
